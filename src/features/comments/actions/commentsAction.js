...
export const getComments = (method, url, page) => (dispatch, getState) => {
    ...
    apiRequest({
        ...
        onSuccess: getCommentsOnSuccess,
        onFailed: getCommentsOnFailed,
        myFunc: getComments(method, url, pageNum, prevData),
        extraData: { pageNum: pageNum, preData, preData }
    })
}
...
export const sendComment = (url, text, listData, replyToId, replyIndex) => (dispatch) => {
    ...
    apiRequest({
        ...
        url: url,
        params : formData,
        onSuccess: sendCommentOnSuccess,
        onFailed: sendCommentOnFailed,
        myFunc: sendComment(url, text, listData, replyToId),
        extraData: { listData: listData, replyIndex: replyIndex, replyId: replyToId }
    })
}
...
export const getMentionList = (url) => (dispatch) => {
    ...
    apiRequest({
        method: 'get',
        url: url,
        onSuccess: getMentionsOnSuccess,
        onFailed: getMentionsOnFailed,
        myFunc: getMentionList(url),
    })
}
...
export const reactionComment = (url, reaction, listData, parentIndex, subIndex) => (dispatch) => {
    ...
    apiRequest({
        ...
        url: url,
        params: formData,
        onSuccess: reactionCommentOnSuccess,
        onFailed: reactionCommentOnFailed,
        myFunc: reactionComment(url, reaction, listData, parentIndex, subIndex),
        extraData: { listData, reaction, parentIndex, subIndex }
    })
}